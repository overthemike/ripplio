...
const isComputedPath = (
	p: Path | readonly [typeof __COMPUTED_VALUE_KEY, DepKey],
...
const runComputed = (depKey: DepKey): unknown => {
	...
	// RAW deps captured during this run
...
const makeProxy = <T extends object>(target: T, store: Store, base: Path, rootRef: object): T => {
	...
	const p = new Proxy(target as object, {
		get(obj: object, prop: string | symbol, receiver: unknown) {
			...
			if (desc && "get" in desc && typeof desc.get === "function") {
				...
				return createLivePrimitive<Primitive>({ [__COMPUTED_VALUE_KEY]: computedValueRoot }, [
					__COMPUTED_VALUE_KEY,
					depKey,
				])
			}
...
export type Snapshot<T> = T extends LivePrimitive<infer P>
	? P
	: T extends Primitive
		? T
